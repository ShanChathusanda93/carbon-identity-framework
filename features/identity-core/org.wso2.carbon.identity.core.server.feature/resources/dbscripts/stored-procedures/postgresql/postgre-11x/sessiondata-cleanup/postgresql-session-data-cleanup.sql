...
-- ------------------------------------------
batchSize := 1; -- SET BATCH SIZE FOR AVOID TABLE LOCKS    [DEFAULT : 10000]
chunkSize := 10; -- CHUNK WISE DELETE FOR LARGE TABLES     [DEFAULT : 500000]
checkCount := 2; -- SET CHECK COUNT FOR FINISH CLEANUP SCRIPT (CLEANUP ELIGIBLE RECORDS COUNT SHOULD BE HIGHER THAN checkCount TO CONTINUE) [DEFAULT : 500]
...
enableAudit := TRUE;  -- SET TRUE FOR  KEEP TRACK OF ALL THE DELETED RECORDS USING A TABLE  [DEFAULT : FALSE]
enableOperationDelete := TRUE;
...
sessionCleanupTime := (unix_timestamp*1000000000) - (720*60000000000);
...
-- ------------------------------------------------------
-- DELETE IDN_AUTH_SESSION_STORE SESSION DATA CALCULATION
-- ------------------------------------------------------

IF (enableLog) THEN
RAISE NOTICE 'CALCULATING PURGING RECORDS ON TABLE IDN_AUTH_SESSION_STORE.... ! ';

IF (enableLog AND logLevel IN ('TRACE','DEBUG')) THEN
SELECT COUNT(1) INTO rowcount FROM idn_auth_session_store;
RAISE NOTICE 'TOTAL RECODES ON IDN_AUTH_SESSION_STORE TABLE BEFORE DELETE : %', rowcount;
END IF;

IF (enableLog AND logLevel IN ('TRACE')) THEN
SELECT COUNT(1) FROM (SELECT session_id FROM idn_auth_session_store WHERE time_created < sessionCleanupTime) AS A INTO cleanupCount;
RAISE NOTICE 'TOTAL RECORDS ON IDN_AUTH_SESSION_STORE SHOULD BE DELETED  : %', cleanupCount;
END IF;

IF (enableLog AND logLevel IN ('TRACE')) THEN
rowcount := (rowcount - cleanupCount);
RAISE NOTICE 'TOTAL RECORDS ON IDN_AUTH_SESSION_STORE SHOULD BE RETAIN  : %', rowcount;
END IF;
END IF;

-- ------------------------------------------------------
-- DELETE IDN_AUTH_SESSION_STORE OPERATIONAL DATA CALCULATION
-- ------------------------------------------------------
...
IF (enableLog AND logLevel IN ('TRACE')) THEN
SELECT COUNT(1) FROM (SELECT session_id FROM idn_auth_session_store WHERE operation = 'DELETE' and time_created < operationCleanupTime) AS A INTO cleanupCount;
RAISE NOTICE 'TOTAL RECODES ON IDN_AUTH_SESSION_STORE SHOULD BE DELETED  : %', cleanupCount;
END IF;

IF (enableLog AND logLevel IN ('TRACE')) THEN
rowcount := (rowcount - cleanupCount);
RAISE NOTICE 'TOTAL RECODES ON IDN_AUTH_SESSION_STORE SHOULD BE RETAIN  : %', rowcount;
END IF;
END IF;

-- ------------------------------------------------------
-- BATCH IDN_AUTH_SESSION_STORE SESSION DATA DELETE
-- ------------------------------------------------------

purgingTable := 'idn_auth_session_store';
--purgingSessionUserMappingTable = 'idn_auth_user_session_mapping';
purgingChunkTable := 'idn_auth_session_store_chunk';
purgingBatchTable := 'idn_auth_session_store_batch';
auditTable := 'idn_auth_session_store_auditlog';
purgeBseColmn := 'session_id';
purgeBseColmnType := 'varchar';
...
IF (rowcount = 1)
THEN
    SELECT COUNT(1) FROM idn_auth_session_store_chunk INTO chunkCount;
    IF (chunkCount = 0)
    THEN
        IF (enableLog AND logLevel IN ('TRACE')) THEN
        RAISE NOTICE 'DROPPING TABLE IDN_AUTH_SESSION_STORE_CHUNK HENCE COUNT IS : %', chunkCount;
        END IF;
...
THEN
    ...
    THEN
    RAISE NOTICE 'PROCEED WITH NEW CHUNK TABLE IDN_AUTH_SESSION_STORE_CHUNK';
    END IF;
...
-- Below query is converted to the 11.x style.
--    EXECUTE 'insert into '||quote_ident(purgingChunkTable)||' ('||quote_ident(purgeBseColmn)||') '||purgingCondition||' limit '||chunkSize;
    INSERT INTO idn_auth_session_store_chunk (session_id) SELECT session_id FROM idn_auth_session_store WHERE time_created < sessionCleanupTime limit chunkSize;
    GET diagnostics chunkCount := ROW_COUNT;
    COMMIT;
    RAISE NOTICE 'CHUNK COUNT %', chunkCount;

    IF (chunkCount < checkCount)
    THEN
      IF (enableLog AND logLevel IN ('TRACE')) THEN
      RAISE NOTICE 'IDN_AUTH_SESSION_STORE DELETE FINISHED HENCE NEW CHUNK CREATED WITH % AND ITS LESS THAN THE CHECK COUNT DEFINED : %', chunkCount, checkCount;
      END IF;
    ...
    ELSE
      IF (enableLog AND logLevel IN ('TRACE','DEBUG')) THEN
      RAISE NOTICE 'CHUNK TABLE IDN_AUTH_SESSION_STORE_CHUNK CREATED WITH : %', chunkCount;
      END IF;
      IF (enableAudit)
      THEN
...
ELSE
    IF (enableLog AND logLevel IN ('TRACE')) THEN
    RAISE NOTICE 'PROCEED WITH EXISTING CHUNK TABLE IDN_AUTH_SESSION_STORE_CHUNK WITH : %', chunkCount;
    END IF;
END IF;
...
THEN
-- This segment has to be looped IMO. In this setup only one time execution is happen.
    EXECUTE 'SELECT count(1) from pg_catalog.pg_tables WHERE schemaname = current_schema() AND tablename =  $1' into rowcount USING purgingBatchTable;
    IF (rowcount = 1)
    THEN
    EXECUTE 'DROP TABLE '||quote_ident(purgingBatchTable);
    ...
    THEN
        IF (enableLog AND logLevel IN ('TRACE')) THEN
        RAISE NOTICE 'BATCH DELETE START ON TABLE IDN_AUTH_SESSION_STORE WITH : %', batchCount;
        END IF;
        ...
        RAISE NOTICE 'BATCH DELETE FINISHED ON TABLE IDN_AUTH_SESSION_STORE WITH : %', deleteCount;
        END IF;

--        IF (enableOperationDelete)
--        THEN
...
IF (enableLog) THEN
RAISE NOTICE '';
RAISE NOTICE 'WSO2_SESSION_DATA_CLEANUP_SP COMPLETED .... !';
RAISE NOTICE '';
END IF;
